...
public class PersonalProfileServiceImpl implements PersonalProfileService {
	...
	public boolean save(PersonalProfile personalProfile) {
		 ...
		 String housing_condition_now=personalProfile.getHousing_condition_now();  // 0房改/继承 1按揭自置2无按揭自置 3与父母同住 4租借// 5公司提供 6其他
		 ...
		 String start=personalProfile.getstart();  // 状态  0按揭员录单1待初审审批中2待终审审批中3待出账确认4待放款5待取证6待解押7待进押8待确认回款9待结算10已结清
		 String ctime=personalProfile.getCtime();  // 创建时间 

		 
		 JointApplicant joint = new JointApplicant(
				 name, phoneticize, id_type,id_number,
	...
	@Override
	public boolean update(PersonalProfile personalProfile) {
		String name=personalProfile.getName(); // 共同借款人姓名
		 String phoneticize=personalProfile.getPhoneticize();  // 拼音、英文姓名
		 String id_type=personalProfile.getId_type();  // 身份证件类型
		 String id_number=personalProfile.getId_number(); // 身份证件号码
		 ...
		 PersonalProfile joinss = new PersonalProfile(
				 name, phoneticize, id_type,id_number,
					relationship_with_borrower,  country_and_region,  sex, Local_domicile,
					household_registration, marital_status,  housing_condition_now, birthday,
					home_address_now,  home_phone, mobile_phone,email,
					...
					Rent_income,  Other_income, family_number,monthly_expenditure,
					 postal_address,  start, ctime);
		 boolean isResult = personalProfiledao.update(joinss);
		 if (isResult == true) {
			return true;
		}else {
			return false;
	...
	public boolean delete(int id) {
		boolean isResult = personalProfiledao.delete(id);
		if (isResult == true) {
			return true;
		}else {
			return false;
	...
	@Override
	public PersonalProfile findById(int id) {
		// TODO Auto-generated method stub
		return null;
	}

	@Override
	public List<PersonalProfile> findAll() {
		// TODO Auto-generated method stub
		return null;
	}
