...
var initData = function() {
	var method = 'GET'
	var url = '/slloan/loan/ObtainEvidence?page='+init.pages+'&limit='+ init.limit
	var datas = {}
			...
			layer.open({
				  title: '上传凭证'
					  ,content: '<form  id="frm-reg"  enctype="multipart/form-data"><fieldset>'+
						'<legend>上传凭证</legend>'+
						'上传类型 <input type="text" id="uploadtype1" value="取证凭证" name="upload_type" /><br>'+
...
function updateUserInfo() {
    ...
    $.ajax({  
    url: "/slloan/financevoucher/forensics",
    type:"post",  
    data:form,  
    cache: false,  
    processData: false,  
